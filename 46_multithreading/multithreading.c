...
bool init() {
  ...
  // create window
  gWindow = LWindow_new("46 - Multithreading", SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0);
  if (gWindow == NULL) {
...
bool setup()
{
  ...
  // create then run the thread
  thread_id = SDL_CreateThread(thread_function, "LazyThread", (void*)data);
