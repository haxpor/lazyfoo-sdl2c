...
const int k_rendertarget_width = SCREEN_WIDTH/2;
const int k_rendertarget_height = SCREEN_HEIGHT/2;
LTexture* rendertarget_texture = NULL;
double angle = 0;
SDL_Point rendertarget_center = { k_rendertarget_width/2, k_rendertarget_height/2 };
...
bool init() {
  ...
  // create window
  gWindow = LWindow_new("43 - Render to Texture", SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0);
  if (gWindow == NULL) {
...
bool setup()
{
  ...
  rendertarget_texture = LTexture_NewBlankRenderTarget(k_rendertarget_width, k_rendertarget_height, SDL_PIXELFORMAT_RGBA8888);
  if (rendertarget_texture == NULL)
  {
...
void render(float deltaTime)
{
  ...
  // set our rendertarget texture as render target
  LTexture_SetAsRenderTarget(rendertarget_texture);

  // clear screen
  SDL_SetRenderDrawColor(gWindow->renderer, 0x00, 0xff, 0xff, 0xff);
  SDL_RenderClear(gWindow->renderer);

  // render red filled red
  SDL_Rect fill_rect = { k_rendertarget_width/4, k_rendertarget_height/4, k_rendertarget_width/2, k_rendertarget_height/2 };
  SDL_SetRenderDrawColor(gWindow->renderer, 0xff, 0x00, 0x00, 0xff);
  SDL_RenderFillRect(gWindow->renderer, &fill_rect);

  // render green outlined quad
  SDL_Rect outline_rect = { 0, 0, k_rendertarget_width, k_rendertarget_height };
  SDL_SetRenderDrawColor(gWindow->renderer, 0x00, 0xff, 0x00, 0xff);
  SDL_RenderDrawRect(gWindow->renderer, &outline_rect);

  // render blue horizontal line
  SDL_SetRenderDrawColor(gWindow->renderer, 0x00, 0x00, 0xff, 0xff);
  SDL_RenderDrawLine(gWindow->renderer, 0, k_rendertarget_height/2, k_rendertarget_width, k_rendertarget_height/2);

  // set render target back to normal window
  SDL_SetRenderTarget(gWindow->renderer, NULL);

  // show rendered to texture
  LTexture_RenderEx(rendertarget_texture, SCREEN_WIDTH/2 - k_rendertarget_width/2, SCREEN_HEIGHT/2 - k_rendertarget_height/2, 1.0, angle, &rendertarget_center, SDL_FLIP_NONE);
}
...
void close()
{
  ...
  if (rendertarget_texture != NULL)
    LTexture_Free(rendertarget_texture);
