...
SDL_SpinLock spinlock_datalock = 0;
int tshared_data = -1;
...
int worker(void *data)
{
  ...
  for (int i=0; i<5; i++)
  {
    ...
    SDL_AtomicLock(&spinlock_datalock);
    ...
    // unlock
    SDL_AtomicUnlock(&spinlock_datalock);
...
bool init() {
  ...
  // create window
  gWindow = LWindow_new("48 - Atomic Operations", SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0);
  if (gWindow == NULL) {
