...
DataStream* feeder_stream;
// texture we will update its pixel data from source of stream
LTexture* streaming_texture;
...
bool init()
{
  ...
  // create window
  gWindow = LWindow_new("42 - Texture Streaming", SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0);
  if (gWindow == NULL) {
...
bool setup()
{
  ...
  // feeder stream
  feeder_stream = DataStream_new();
  if (feeder_stream == NULL)
  {
    SDL_Log("Failed to create datastream");
    return false;
  }

  // streaming texture
  streaming_texture = LTexture_NewBlank(feeder_stream->current_image->w, feeder_stream->current_image->h, SDL_PIXELFORMAT_RGBA8888);
...
void update(float deltaTime)
{
  DataStream_update(feeder_stream, deltaTime);
}
...
void render(float deltaTime)
{
  if (!gWindow->is_minimized)
  {
    ...
    // lock texture
    LTexture_LockTexture(streaming_texture);
    // copy pixel data from feeder to it
    LTexture_CopyPixels(streaming_texture, feeder_stream->current_image->pixels);
    // unlock texture
    LTexture_UnlockTexture(streaming_texture);

    LTexture_Render(streaming_texture, SCREEN_WIDTH/2 - streaming_texture->width/2, SCREEN_HEIGHT/2 - streaming_texture->height/2);
  }
}
...
void close()
{
  ...
  // datastream
  DataStream_free(feeder_stream);

  // streaming texture
  LTexture_Free(streaming_texture);
