...
bool init() {
  ...
  // create window
  gWindow = SDL_CreateWindow("22 - Timing", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
  if (gWindow == NULL) {
...
bool setup()
{
  ...
  // load prompt texture
  gPromptTexture = LTexture_LoadFromRenderedText("Press Enter to Reset Start Time", gTextColor, 0);
...
void update()
{
  // as we keep track of start time, use SDL_GetTicks() will get us relative time going on
  snprintf(timerText, BUFFER-1, "Milliseconds since start time %d", SDL_GetTicks() - startTime);
}
  ...
  else if (e->type == SDL_KEYDOWN)
  {
    switch (e->key.keysym.sym)
    {
      case SDLK_ESCAPE:
        ...
        // reset start time on return key
      case SDLK_RETURN:
