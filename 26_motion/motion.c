 ...
 *  - use DOT_SPEED which is pixel per second, not per frame
 *  - smooth movement via lerp function (note need to lerp on velocity)
 */
...
bool init() {
  ...
  // create window
  gWindow = SDL_CreateWindow("26 - Motion", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
  if (gWindow == NULL) {
...
void update(float deltaTime)
{
  ...
  // bound position
  if (dot.posX < 0 || dot.posX + dot.texture->width > SCREEN_WIDTH)
  {
    // move back
    dot.posX -= dot.velX;
  }
  if (dot.posY < 0 || dot.posY + dot.texture->height > SCREEN_HEIGHT)
  {
    // move back
    dot.posY -= dot.velY;
  }
