...
bool init() {
  ...
  // create window
  gWindow = SDL_CreateWindow("31 - Scrolling Backgrounds", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
  if (gWindow == NULL) {
...
bool setup()
{
  ...
  if (bgTexture == NULL)
  {
    SDL_Log("Failed to load bg.png: %s", SDL_GetError());
    return false;
  }
  ...
  Dot_Init(&dot, 50, SCREEN_HEIGHT/2, dotTexture);
...
void render(float deltaTime)
{
  ...
  LTexture_Render(bgTexture, bg_offset, 0);
  LTexture_Render(bgTexture, bg_offset + bgTexture->width, 0);
  Dot_Render(&dot);
}
