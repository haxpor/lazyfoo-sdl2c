/**
 * 44 - Framerate Independent Movement
 *
 * Note: See README.md for important note about this sample.
 */
...
bool setup()
{
  ...
  // dot texture
  dot_texture = LTexture_LoadFromFile("dot.bmp");
  if (dot_texture == NULL)
  {
    SDL_Log("failed to create texture for dot.bmp");
    return false;
  }
  ...
  // setup bound system
  // level dimension is exactly the screen dimension
  BoundSystem_source_init(SCREEN_WIDTH, SCREEN_HEIGHT);
