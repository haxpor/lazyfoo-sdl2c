...
{
  // interval is in ms
  SDL_Log("Callback called back with message: %s [interval: %d]", (char*)param, interval);
  return 0;
}
...
bool init() {
  ...
  // create window
  gWindow = LWindow_new("45 - Timer Callbacks", SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0);
  if (gWindow == NULL) {
...
bool setup()
{
  ...
  {
    SDL_Log("failed to create texture from splash.texture");
    return false;
  }

  // setup to call our callback after 3 seconds
  // callback function will be run on different *thread*
  timer_id = SDL_AddTimer(3 * 1000, callback, "3 seconds waited!");
...
void close()
{
  ...
  // remove timer in case callback is not called
  // note: yep if callback called, then it will be removed automatically
  if (SDL_RemoveTimer(timer_id))
    SDL_Log("removed timer [timer id: %d]", timer_id);
