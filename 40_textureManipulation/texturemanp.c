...
bool init() {
  ...
  // create window
  gWindow = LWindow_new("Texture Manipulation", SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0);
  if (gWindow == NULL) {
...
bool setup()
{
  ...
  // foo texture
  foo_texture = LTexture_LoadFromFileWithColorKeyEx("foo.png", false, 0xff, 0xff, 0xff, SDL_TEXTUREACCESS_STREAMING);
  if (foo_texture == NULL)
  {
    SDL_Log("Failed to load foo.png texture");
    return false;
  }

  // lock foo texture
  if (!LTexture_LockTexture(foo_texture))
  {
    SDL_Log("Cannot lock foo texture");
    return false;
  }
  // locked successfully
  // do manual color key
  Uint32 format = SDL_GetWindowPixelFormat(gWindow->window);
  SDL_PixelFormat* mapping_format = SDL_AllocFormat(format);
  // show pixel format in human readable string
  // it creates with ARGB8888, if we want to change we need to set hint before SDL creating renderer
  SDL_Log("pixel format name: %s", SDL_GetPixelFormatName(format));

  // get pixel data
  Uint32* pixels = (Uint32*)foo_texture->pixels;
  // convert pitch into pixels by divide by 4 as each pixel has 4 components 1 bytes each thus 4 bytes each pixel
  int pixel_count = foo_texture->pitch / 4 * foo_texture->height;
  ...
  for (int i=0; i<pixel_count; i++)
  {
    ...
    {
      pixels[i] = to_swap_color;
    }
  }

  // unlock texture
  LTexture_UnlockTexture(foo_texture);
  // free format
...
void render(float deltaTime)
{
  if (!gWindow->is_minimized)
  {
    ...
    LTexture_Render(foo_texture, SCREEN_WIDTH/2-foo_texture->width/2, SCREEN_HEIGHT/2-foo_texture->height/2);
  }
}
