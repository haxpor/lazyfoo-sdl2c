...
bool init() {
  ...
  // create window
  gWindow = SDL_CreateWindow("29 - Circular Collision Detection", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
  if (gWindow == NULL) {
...
void update(float deltaTime)
{
  ...
  Dot_UpdateCollision_Circle(&dotA, &dotB->collider);
